...
def _real_agent(*, session_id: str = "plugin-section-test") -> AIAgent:
    # Exercise the real prompt/runtime, without unrelated network metadata or
    # tool-backend discovery during agent construction.
    with (
...
def test_real_aiagent_builds_section_once_and_preserves_cached_bytes(monkeypatch):
    calls = []
...
def test_legacy_saved_prompt_is_not_reconciled_with_current_plugin(monkeypatch):
    ...
    calls = []
    manager = PluginManager()
    _install_test_section(manager, lambda info: calls.append(info) or "new rules")
    monkeypatch.setattr(plugins, "_plugin_manager", manager)
    saved = "Original prompt without any plugin section.\n\nConversation started: earlier"
    agent = SimpleNamespace(_cached_system_prompt=saved)
    restore_plugin_prompt_sections(agent, saved)
    assert _frozen_plugin_prompt_sections(agent) == ()
    assert calls == []
    assert agent._cached_system_prompt == saved
